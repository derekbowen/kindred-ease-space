...
function PageAuditorPage() {
  ...
  const [suggestions, setSuggestions] = useState<Array<{ url_path: string; title: string | null; status: string }>>([]);
  ...
  async function runAudit() {
    ...
    try {
      ...
      if (r.ok) { setAudit(r.audit); await reload(workspaceId); }
      else { setError(r.error); if ("suggestions" in r && r.suggestions) setSuggestions(r.suggestions); }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">AI scores any of your published pages 0–100 with strengths, weaknesses, and recommendations.</p>
      </div>

      <Card>
        <CardHeader><CardTitle>Audit a page</CardTitle><CardDescription>Paste a path like <code>/p/los-angeles-ca</code>.</CardDescription></CardHeader>
            ...
            <div className="rounded-md border border-destructive/50 bg-destructive/10 p-3 text-sm text-destructive">
              ...
              {suggestions.length > 0 && (
                <ul className="mt-2 space-y-1 text-xs">
                  {suggestions.map((s) => (
                    <li key={s.url_path}><button className="underline" onClick={() => setPath(s.url_path)}>{s.url_path}</button> — {s.title || "(untitled)"} [{s.status}]</li>
                  ))}
                </ul>
              )}
            </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="text-4xl font-bold">{audit.score ?? "—"}<span className="text-base text-muted-foreground">/100</span></div>
            </div>
          </CardHeader>
      ...
      <Card>
        <CardHeader><CardTitle>Recent audits</CardTitle><CardDescription>{recent.length} rows</CardDescription></CardHeader>
        <CardContent>
          {recent.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead className="border-b text-left text-xs uppercase text-muted-foreground">
                <tr><th className="py-2 pr-4">When</th><th className="py-2 pr-4">URL</th><th className="py-2 pr-4">Score</th><th className="py-2">Summary</th></tr>
              </thead>
              <tbody>
                {recent.map((r) => (
                  <tr key={r.id} className="border-b last:border-0">
                    <td className="py-2 pr-4 text-xs text-muted-foreground">{new Date(r.audited_at).toLocaleString()}</td>
