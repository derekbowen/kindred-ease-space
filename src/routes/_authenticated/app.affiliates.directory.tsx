...
function AffiliatesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      listAffiliates({ data: { workspaceId: workspaceId!, search: search || undefined } }),
    enabled: !!workspaceId,
  });
  ...
  const onCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!workspaceId || !newProgram) {
      toast.error("Pick a program first.");
      return;
    }
    try {
      await create({
        data: { workspaceId, programId: newProgram, name: newName, email: newEmail },
      });
      setNewName("");
      setNewEmail("");
      await refresh();
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Add an affiliate</CardTitle>
        </CardHeader>
        <CardContent>
