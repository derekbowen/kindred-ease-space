...
function GscImportPage() {
  ...
  useEffect(() => {
    if (workspaceId) loadStats(workspaceId); /* eslint-disable-next-line */
  }, [workspaceId]);
  ...
  async function doImport() {
    ...
    if (!rows.length) {
      setMsg(
        "No rows parsed. Make sure the CSV has Page, Query, Clicks, Impressions, CTR, Position columns.",
      );
      return;
    }
    setBusy(true);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Paste a Google Search Console query export to feed the keyword opportunity finder.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Headers expected: Page (or URL), Query, Clicks, Impressions, CTR, Position. Tab- or
            comma-separated.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center gap-3">
            <Button
              onClick={doImport}
              disabled={busy || !workspaceId || !csv.trim()}
              className="gap-2"
            >
