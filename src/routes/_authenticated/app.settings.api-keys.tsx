...
function ApiKeysPage() {
  ...
  async function save() {
    ...
    try {
      const r = await upsert({ data: { workspaceId, keyName: keyName.trim().toUpperCase(), value: value.trim() } });
      setMsg(r.ok ? "Saved." : `Error: ${r.error}`);
      if (r.ok) { setValue(""); await reload(workspaceId); }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Owner-only. Values are server-side; the UI never displays the full secret again.</CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{rows.length} key{rows.length === 1 ? "" : "s"}</CardDescription>
        </CardHeader>
        <CardContent>
          {rows.length === 0 ? (
            ...
            <table className="w-full text-sm">
              <thead className="border-b text-left text-xs uppercase text-muted-foreground">
                <tr><th className="py-2 pr-4">Key</th><th className="py-2 pr-4">Preview</th><th className="py-2 pr-4">Updated</th><th className="py-2"></th></tr>
              </thead>
              <tbody>
                {rows.map((r) => (
