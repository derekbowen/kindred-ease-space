...
function AffiliateSettings() {
    ...
    getMe()
      .then(async (me) => {
        ...
        if (ws) {
          ...
          const s = r.settings;
          setFormSlug(s.form_slug ?? "");
          setBaseUrl(s.marketplace_base_url ?? "");
          setCurrency(s.currency ?? "USD");
          setParam(s.referrer_param ?? "referrerID");
        }
      })
      .catch(() => {});
  }, [load]);
  ...
  return (
    <div className="max-w-xl space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Marketplace & links</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={onSave} className="space-y-4">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Public sign-up page:{" "}
                <span className="font-mono">/apply/{formSlug || "{slug}"}</span>
              </p>
            </div>
            <div className="space-y-1">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Affiliate links append <span className="font-mono">?{param}=CODE</span> to this URL.
              </p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <Label>Referral URL parameter</Label>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Pulls transactions from your connected Sharetribe marketplace, attributes them to
            affiliates via the
            <span className="font-mono"> {param}</span> stored in the referred user's private data,
            and accrues payouts.
          </CardDescription>
        </CardHeader>
        <CardContent>
