...
function PayoutsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      listPayouts({ data: { workspaceId: workspaceId!, status: tab === "all" ? undefined : tab } }),
    enabled: !!workspaceId,
  });
      ...
      <div className="grid grid-cols-3 gap-4">
        {(["pending", "ready", "paid"] as const).map((s) => (
          <Card key={s}>
            ...
            <CardContent className="text-2xl font-bold">{counts[s] ?? 0}</CardContent>
          </Card>
        ))}
      </div>
              ...
              {data!.payouts.map((p) => (
                <tr key={p.id} className="border-t border-border">
                  ...
                  <td className="p-2 text-right space-x-1 whitespace-nowrap">
                    {p.status === "pending" && (
                      <Button size="sm" variant="outline" onClick={() => act(p.id, "ready")}>
                        Approve
                      </Button>
                    )}
                    {p.status === "ready" && (
                      <Button size="sm" onClick={() => act(p.id, "paid")}>
                        Mark paid
                      </Button>
                    )}
                    {(p.status === "pending" || p.status === "ready") && (
                      <Button size="sm" variant="ghost" onClick={() => act(p.id, "rejected")}>
                        Reject
                      </Button>
                    )}
                  </td>
