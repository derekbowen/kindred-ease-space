  ...
  const send = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        while (!done) {
          ...
          while ((nl = buffer.indexOf("\n")) !== -1) {
            ...
            if (payload.startsWith("[") && !sources) {
              try {
                sources = JSON.parse(payload) as Source[];
                continue;
              } catch {
                /* fall through */
              }
        ...
        setMessages((prev) =>
          prev.map((m) => (m.id === botId ? { ...m, isStreaming: false, sources } : m)),
        );
      } catch (err) {
            ...
            {messages.length === 0 ? (
              <div className="flex flex-col items-center text-center pt-6">
                ...
                <p className="text-xs text-muted-foreground mb-5 px-2">
                  Grounded in our help center. I cite the articles I use.
                </p>
                <div className="flex flex-col gap-2 w-full">
                  {SUGGESTIONS.map((s) => (
                    <button
                      key={s}
                      onClick={() => send(s)}
                      className="text-left px-3 py-2.5 rounded-lg border border-border bg-background hover:border-primary/40 hover:bg-accent transition text-[13px]"
                    >
                      {s}
                    </button>
                  ))}
