...
export const updateAffiliateSettings = createServerFn({ method: "POST" })
    ...
    z
      .object({
        ...
        referrerParam: z
          .string()
          .trim()
          .regex(/^[a-zA-Z][a-zA-Z0-9_-]*$/, "Must start with a letter; letters/numbers/-/_ only")
          .max(40)
          .optional(),
      })
      .parse(d),
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.marketplaceBaseUrl !== undefined)
      patch.marketplace_base_url = data.marketplaceBaseUrl || null;
...
export const listPrograms = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const a of (affs ?? []) as Array<{ program_id: string }>)
      counts.set(a.program_id, (counts.get(a.program_id) ?? 0) + 1);
    return {
...
export const upsertProgram = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if ((count ?? 0) >= limit) {
      throw new Error(
        `Your plan allows ${limit} program${limit === 1 ? "" : "s"}. Upgrade to add more.`,
      );
    }
...
export const listAffiliates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = sb()
      .from("affiliates")
      .select(
        "id, program_id, name, email, referral_code, status, created_at, affiliate_programs:program_id(name)",
      )
      .eq("workspace_id", data.workspaceId)
      .order("created_at", { ascending: false })
      .limit(500);
    ...
    const { data: txns } = await sb()
      ...
      .select("affiliate_id, gmv, marketplace_revenue")
      .eq("workspace_id", data.workspaceId);
    ...
    const gmv = new Map<string, number>(),
      rev = new Map<string, number>(),
      pay = new Map<string, number>();
    ...
    for (const p of (paid ?? []) as any[])
      pay.set(p.affiliate_id, (pay.get(p.affiliate_id) ?? 0) + Number(p.amount || 0));
    ...
    return {
      affiliates: ((rows ?? []) as any[]).map((a) => ({
        ...
        status: a.status,
        link: base
          ? `${base}${base.includes("?") ? "&" : "?"}${param}=${a.referral_code}`
          : `?${param}=${a.referral_code}`,
        gmv: gmv.get(a.id) ?? 0,
...
export const getAffiliateDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [
      ...
      sb()
        .from("affiliate_transactions")
        .select("gmv, marketplace_revenue, occurred_at, referral_id")
        .eq("workspace_id", data.workspaceId),
    ...
    const totalPayouts = ((payouts ?? []) as any[])
    ...
    for (const t of tx) {
      ...
      if (b) {
        b.gmv += Number(t.gmv || 0);
        b.revenue += Number(t.marketplace_revenue || 0);
      }
    }
    ...
    return {
      ...
      kpis: {
        ...
        active_affiliates: ((affiliates ?? []) as any[]).filter((a) => a.status === "active")
          .length,
...
export const listApplications = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows } = await q;
    return {
      applications: ((rows ?? []) as any[]).map((r) => ({
        ...r,
        program_name: r.affiliate_programs?.name ?? "—",
      })),
    };
  });
...
export const decideApplication = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: unknown) =>
    z
      .object({ workspaceId: workspaceIdSchema, id: z.string().uuid(), approve: z.boolean() })
      .parse(d),
  )
  .handler(async ({ data, context }) => {
    await assertWorkspaceOwner(data.workspaceId, context.userId);
