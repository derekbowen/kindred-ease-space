 ...
 *  This is the pipeline behind approveOpportunity; it checks the owner itself. */
export async function runApproveOpportunity(
  data: z.infer<typeof ApproveOpportunityInputSchema>,
  userId: string,
) {
  await assertAvailable(data.workspaceId);
  await assertWorkspaceOwner(data.workspaceId, userId);
