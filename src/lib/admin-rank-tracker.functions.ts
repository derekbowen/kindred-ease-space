...
export const deleteTrackedKeyword = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertWorkspaceMember(data.workspaceId, context.userId);
    const { error } = await sb().from("tracked_keywords")
      .delete()
      .eq("workspace_id", data.workspaceId)
      .eq("id", data.id);
    return error ? { ok: false as const, error: error.message } : { ok: true as const };
...
export const runSerpCheck = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: ws } = await sb()
      .from("workspaces").select("marketplace_domain").eq("id", data.workspaceId).maybeSingle();
    const targetDomain: string = (ws?.marketplace_domain || "").replace(/^https?:\/\//, "").replace(/\/+$/, "").toLowerCase();
    if (!targetDomain) return { ok: false as const, error: "Workspace has no marketplace_domain set." };

    let q = sb().from("tracked_keywords").select("*")
      .eq("workspace_id", data.workspaceId)
      .eq("is_active", true);
    ...
    for (const kw of (kws || []) as TrackedKeywordRow[]) {
      try {
        const params = new URLSearchParams({
          engine: "google", q: kw.keyword, gl: kw.market || "us", hl: "en", num: "100", api_key: serpKey,
        });
        const resp = await fetch(`https://serpapi.com/search.json?${params.toString()}`);
        if (!resp.ok) {
          results.push({ keyword: kw.keyword, position: null, delta: null, error: `serpapi ${resp.status}` });
          continue;
        }
        ...
        await sb().from("serp_rankings").insert({
          workspace_id: data.workspaceId,
          keyword_id: kw.id, position, url_found: urlFound, checked_at: now,
        });
