// Stripe Checkout session creator (subscription or credit top-up)
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { workspace_id, mode, price_id, quantity } = await req.json();
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { workspace_id, mode: mode ?? "subscription" },
      subscription_data: mode === "credits" ? undefined : { metadata: { workspace_id } },
    });
