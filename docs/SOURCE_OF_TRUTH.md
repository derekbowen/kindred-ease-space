...
| Published-page capacity | `workspaces.page_limit_base` + `page_limit_addon` + `page_limit_bonus`, plus active `workspace_entitlement_grants` (a grant supersedes a trial; it adds to a paid plan), all gated by `decideCapacity()` / `workspace_capacity()` on every read | `PageEntitlement.pageLimit` (computed per read) |
